...
def log_step(session: Session, episode_id: int, step_index: int,
             skill_name: str, observation: str,
             p_before: float, p_after: float,
             silver_stamp: Optional[Dict[str, Any]] = None) -> None:
    """Log a single step in an episode.

    This variant keeps the original behaviour *and* optionally attaches
    a "silver gauge" stamp to the Step node.
    ...
    session:
        Neo4j session.
    episode_id:
        Internal Neo4j id of the Episode node.
    step_index:
        Zero-based index of the step within the episode.
    skill_name:
        Name of the Skill node used.
    observation:
        Name of the Observation node perceived.
    p_before / p_after:
        Belief about the door being unlocked before/after this step.
    silver_stamp:
        Optional dictionary produced by `scoring_silver.build_silver_stamp`.
        If not provided, we will *try* to construct one on the fly using
        the Skill.cost value and the belief `p_before`. If the
        `scoring_silver` module isn't present, or anything goes wrong,
        this gracefully falls back to plain logging.
    """

    # Try to construct a silver stamp if not supplied.
    # This keeps the patch backwards compatible: existing callers that
    # don't know about the silver gauge don't need to change anything.
    if silver_stamp is None:
        try:
            ...
            try:
                # We need the skill cost to feed into the existing scoring.
                with session.begin_transaction() as tx_cost:
                    res = tx_cost.run(
                        """
                        MATCH (sk:Skill {name: $skill_name})
                        RETURN coalesce(sk.cost, 1.0) AS cost
                        """,
                        skill_name=skill_name,
                    )
                    rec = res.single()
                    cost = rec["cost"] if rec is not None else 1.0

                silver_stamp = build_silver_stamp(skill_name, float(cost), float(p_before))
                silver_json = json.dumps(silver_stamp)
                silver_score = float(silver_stamp.get("silver_score"))
            except Exception:
                # Fail soft: don't block episode logging if something goes wrong
                silver_stamp = None
                silver_json = None
                silver_score = None
    else:
        try:
            silver_json = json.dumps(silver_stamp)
            silver_score = float(silver_stamp.get("silver_score")) if isinstance(silver_stamp, dict) else None
        except Exception:
            silver_json = None
            silver_score = None

    def _create_step_tx(tx, episode_id, agent_name, step_index,
        ...
        MERGE (s)-[:OBSERVED]->(obs)
        SET s.p_before = $p_before,
            s.p_after = $p_after,
            s.created_at = datetime(),
            s.skill_name = $skill_name
        """

        # Only attach silver metadata if we have it.
        if silver_json is not None:
            query += """
            SET s.silver_stamp = $silver_json,
